...
class Wordsearch():
	...
	def initialize_empty_board(self):
		...
		"""
		self.grid = [['-']*self.size for n in range(self.size)]
	...
	def place_another_word(self):
		...
		while czy_dolozono == False:
			...
			def choose_word():
				"""
				Losuje słowo aż znajdzie takie, którego jeszcze nie ma na planszy
				"""
				
				def choose():
					global losowy_index
					losowy_index = randint(0,self.size)
					try:
	...
	def to_uppercase(self):
		"""
		Zmienia małe litery na planszy i na liście słów na duże
		"""
		for i in range(0,self.size):
			for j in range(0,self.size):
				self.grid[i][j] = self.grid[i][j].upper()
		
		for j in range(0,len(self.chosen_words)):
			self.chosen_words[j] = self.chosen_words[j].upper()
...
def create_wordsearch(size):
	...
	for word in w.chosen_words:
		w.full_word_list.append(word)
	#w.to_uppercase()
	return w
